...
import { listVaults } from '@/application/vault';
import { run } from '@/composition';
import { vaultRefFullName, type Vault } from '@/domain/vault';

import { Link } from '@/ui/components/link';
import { vaultRoutePath } from '@/ui/router';
import type { ToastAction } from '@/ui/toast';
import { isSessionExpiredError, vaultErrorMessage } from '@/ui/vault-error';

export type VaultPickerScreenProps = {
...
export function VaultPickerScreen({
  notify,
  onSessionExpired,
}: VaultPickerScreenProps): JSX.Element {
  const [state, setState] = useState<PickerState>({ kind: 'loading' });

  const load = useCallback(async (): Promise<void> => {
    setState({ kind: 'loading' });
    try {
      const vaults = await run(listVaults);
