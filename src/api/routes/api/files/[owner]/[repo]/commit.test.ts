 ...
 *
 * M4 の R2 先行化: 初期同期済み（R2 メタあり）の Vault は GitHub API を消費せず
 * R2（notes / raw / tree）へだけ反映する。未同期 Vault は従来どおり GitHub へ
...
function postContext(
  ...
  bucket?: R2Bucket,
): Parameters<typeof handleCommitPost>[0] {
  const env = bucket === undefined ? ENV : ({ ...ENV, VAULT_BUCKET: bucket } as unknown as Env);
  return { env, request, params } as unknown as Parameters<typeof handleCommitPost>[0];
}
...
describe('POST /api/files/:owner/:repo/commit', () => {
  it('create/update/delete を Trees/Blobs API で単一コミットにする', async () => {
    ...
    const response = await handleCommitPost(
      postContext(
        postRequest({
  ...
  it('copy は blob sha を再利用して複製先に置き、元パスは残す', async () => {
    mockGitHubSequence();
    ...
    expect(response.status).toBe(200);
    // copy は Blob 作成が発生しない（sha 再利用のため blobs 呼び出しなし）
    const calls = mocks.githubApiFetch.mock.calls.map(
      (call) => (call[3] as { method?: string } | undefined)?.method ?? 'GET',
    );
    expect(calls.filter((method) => method === 'POST').length).toBe(2); // trees + commits

    const treeBody = callBody(3); // POST /git/trees は 4 番目の呼び出し
    const entries = treeBody.tree as { path: string; sha: string | null }[];
    ...
    // 元パスは削除されない（move と違い sha: null のエントリが出ない）
    expect(entries.some((entry) => entry.path === 'a.md' && entry.sha === null)).toBe(false);
  });

  it('copy 元が base tree にない場合は 400 invalid_change で中断する', async () => {
    mockGitHubSequence({ moveSourceMissing: true });

    const response = await handleCommitPost(
      postContext(
        postRequest({
          message: 'Duplicate missing.md',
          changes: [{ op: 'copy', path: 'missing.md', to: 'elsewhere.md' }],
        }),
      ),
    );
    ...
    expect(await readJson(response)).toEqual({
      error: 'invalid_change',
      message: '複製元「missing.md」が見つかりません。',
    });
  });
  ...
  it('ボディ不正（未知の op / 不正パス / base64 でない content）は 400', async () => {
    ...
    const results = await Promise.all(
      cases.map(async (body) => {
        const response = await handleCommitPost(postContext(postRequest(body)));
  ...
  it('未認証は 401、プロキシ未設定は 503 を返す', async () => {
    ...
    const unauthorized = await handleCommitPost(
      postContext(
...
/** 同期済み Vault（メタ + ツリー + ノート）を R2 に用意する（R2 先行テスト用） */
async function seedSyncedVault(bucket: R2Bucket): Promise<void> {
  ...
  await writeVaultTree(bucket, 'octocat', 'notes', {
    ...
    entries: [
      { path: 'a.md', type: 'file' },
      { path: 'b.md', type: 'file' },
      { path: 'attachments/logo.png', type: 'file' },
      { path: 'attachments', type: 'directory' },
    ],
  });
  await writeCachedNote(bucket, 'octocat', 'notes', 'a.md', {
    sha: 'sha-a',
    content: '# A\n',
  });
  await writeCachedNote(bucket, 'octocat', 'notes', 'b.md', {
    sha: 'sha-b',
    content: '# B\n',
  });
...
describe('POST /api/files/:owner/:repo/commit（R2 先行: M4）', () => {
  it('create/update/delete を R2 へ反映し、GitHub API を消費しない', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);
    ...
    const response = await handleCommitPost(
      postContext(
        postRequest({
          ...
          changes: [
            { op: 'update', path: 'a.md', content: btoa('# A v2\n') },
            { op: 'create', path: 'notes/new.md', content: btoa('# new\n') },
            { op: 'delete', path: 'b.md' },
          ],
        }),
        PARAMS,
    ...
    // ノート本文と sha（コンテンツハッシュ）が反映されている
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'a.md')).toEqual({
      sha: await sha256Hex('# A v2\n'),
      content: '# A v2\n',
    });
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'notes/new.md')).toEqual({
      sha: await sha256Hex('# new\n'),
      content: '# new\n',
    });
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'b.md')).toBeNull();
    // ツリーにも反映されている（削除分が消え、追加分が入る）
    const tree = await readVaultTree(bucket, 'octocat', 'notes');
    expect(tree?.entries).toEqual([
      { path: 'a.md', type: 'file', sha: null },
      { path: 'attachments/logo.png', type: 'file', sha: null },
      { path: 'notes/new.md', type: 'file', sha: null },
      { path: 'attachments', type: 'directory', sha: null },
      { path: 'notes', type: 'directory', sha: null },
    ]);
    expect(mocks.githubApiFetch).not.toHaveBeenCalled();
  });

  it('非 Markdown の create は raw/ へ拡張子由来の Content-Type で書く', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);
    ...
    const response = await handleCommitPost(
      postContext(
        postRequest({
          message: '画像アップロード',
          changes: [{ op: 'create', path: 'attachments/new.png', content: btoa('png') }],
        }),
        PARAMS,
        bucket,
      ),
    ...
    expect(response.status).toBe(200);
    const raw = await readCachedRaw(bucket, 'octocat', 'notes', 'attachments/new.png');
    expect(raw?.contentType).toBe('image/png');
    expect(new TextDecoder().decode(raw?.body)).toBe('png');
    expect(mocks.githubApiFetch).not.toHaveBeenCalled();
  });

  it('move は本文を引き継いで移動し、元パスを削除する（ノート）', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);
    ...
    const response = await handleCommitPost(
      postContext(
        postRequest({
          message: 'Rename a.md to notes/a.md',
          changes: [{ op: 'move', path: 'a.md', to: 'notes/a.md' }],
        }),
        PARAMS,
        bucket,
      ),
    ...
    expect(response.status).toBe(200);
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'notes/a.md')).toEqual({
      sha: 'sha-a',
      content: '# A\n',
    });
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'a.md')).toBeNull();
    const tree = await readVaultTree(bucket, 'octocat', 'notes');
    expect(tree?.entries).toContainEqual({ path: 'notes/a.md', type: 'file', sha: null });
    expect(tree?.entries).not.toContainEqual({ path: 'a.md', type: 'file' });
  });

  it('move は添付（raw）も Content-Type を保って移動できる', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);
    ...
    const response = await handleCommitPost(
      postContext(
        postRequest({
          message: 'Move attachments/logo.png',
          changes: [{ op: 'move', path: 'attachments/logo.png', to: 'images/logo.png' }],
        }),
        PARAMS,
        bucket,
      ),
    ...
    expect(response.status).toBe(200);
    const moved = await readCachedRaw(bucket, 'octocat', 'notes', 'images/logo.png');
    expect(moved?.contentType).toBe('image/png');
    expect(new TextDecoder().decode(moved?.body)).toBe('png-bytes');
    expect(await readCachedRaw(bucket, 'octocat', 'notes', 'attachments/logo.png')).toBeNull();
  });

  it('copy は複製先に置き、元パスは残す', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);
    ...
    expect(response.status).toBe(200);
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'a copy.md')).toEqual({
      sha: 'sha-a',
      content: '# A\n',
    });
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'a.md')).toEqual({
  ...
  it('move/copy 元が R2 にない場合は 400 invalid_change で中断する', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);

    const move = await handleCommitPost(
      postContext(
        postRequest({
          message: 'Move missing.md',
          changes: [{ op: 'move', path: 'missing.md', to: 'elsewhere.md' }],
    ...
    expect(await readJson(move)).toEqual({
      error: 'invalid_change',
      message: '移動元「missing.md」が見つかりません。',
    });

    const copy = await handleCommitPost(
      postContext(
        postRequest({
          message: 'Copy missing.md',
          changes: [{ op: 'copy', path: 'missing.md', to: 'elsewhere.md' }],
        }),
        PARAMS,
        bucket,
    ...
    expect(await readJson(copy)).toEqual({
      error: 'invalid_change',
      message: '複製元「missing.md」が見つかりません。',
    });
    expect(mocks.githubApiFetch).not.toHaveBeenCalled();
  ...
  it('メタありでもボディ不正は 400 を返す（R2 へは書き込まない）', async () => {
    const bucket = createFakeR2Bucket();
    await seedSyncedVault(bucket);
    ...
    expect(response.status).toBe(400);
    expect(await readJson(response)).toEqual({ error: 'invalid_body' });
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'a.md')).toEqual({
      sha: 'sha-a',
      content: '# A\n',
    });
  ...
  it('メタなし（R2 バケットのみ）の Vault は GitHub コミットの従来動作のまま', async () => {
    const bucket = createFakeR2Bucket();
    mockGitHubSequence();
    ...
    const response = await handleCommitPost(
      postContext(
        postRequest({
          message: 'Create new.md',
          changes: [{ op: 'create', path: 'new.md', content: btoa('# new\n') }],
        }),
        PARAMS,
        bucket,
      ),
    ...
    // GitHub へはコミットされている（git blobs 呼び出しを含む）
    expect(
      mocks.githubApiFetch.mock.calls.some((call) => call[1] === '/repos/octocat/notes/git/blobs'),
    ).toBe(true);
    // R2 には反映されない（未同期のため）
    expect(await readCachedNote(bucket, 'octocat', 'notes', 'new.md')).toBeNull();
  });
});
